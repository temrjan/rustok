...
> ⚠️ **Стек в §§ 4, 6, 9 устарел (2026-08-30).** Документ описывает Tauri 2.0 + Leptos; проект
> перешёл на React Native 2026-04-28, Tauri-слой снят. Продуктовая часть (проблема, аудитория,
> принципы) в силе — техническая требует пересмотра.
>
> **Требует продуктового решения:** § 6 обещает Desktop (macOS/Windows/Linux) «бесплатно через тот
> же Tauri build». Реализация удалена; трек Desktop либо снимается с обещаний, либо возвращается
> на другом основании. Инженер этот выбор не делает.
