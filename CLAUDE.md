...
**Текущая фаза:** **Phase 5 IN PROGRESS** — real wallet UI. Merged: PR #20 (M3a Receive), PR #21 (RPC timeout fix), PR #22 (M3b Send), PR #24 (Issue #23 — alloy `connect_http` panic, Bug A+B closed). First real on-chain ETH transaction broadcast on Sepolia 2026-05-14 from real device (Xiaomi). 41 jest suites / ~215 tests + 227 Rust workspace tests, all green. **Next:** design-token foundation PR.
