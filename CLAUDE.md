...
**Текущая фаза:** **Phase 5 IN PROGRESS** — real wallet UI + visual polish. Merged on `main`: PR #20 (M3a Receive), PR #21 (RPC timeout fix), PR #22 (M3b Send), PR #24 (Issue #23 — alloy `connect_http` panic, Bug A+B closed), **PR #25 design-token foundation 2026-05-15**, **PR #26 theme soften (B graphite dark + off-white light) 2026-05-15**, **PR #27 hero block redesign (BalanceCard embeds ActionRow + shadow card) 2026-05-15**. First real on-chain ETH transaction broadcast on Sepolia 2026-05-14 from real device (Xiaomi). 43 jest suites / 239 tests / 3 skipped + 227 Rust workspace tests, all green. **Active branch:** `feat/activity-screen-m4` — Phase 5 M4 ActivityScreen real (replaces placeholder). C1 committed `e10e4bc` (pendingTxCache + activityStore + chainName export + 24 unit tests), NOT pushed. C2 (ActivityScreen + TransactionRow + ConfirmSendScreen edit + jest renders) + C3 (docs) deferred to next session. Spec at `.claude/specs/2026-05-15-activity-screen-m4.md` (gitignored). **Smoke owed on JFLFG6MZSSL7WCF6 for PR #27 hero redesign** per memory `feedback_smoke_native_deps`.
